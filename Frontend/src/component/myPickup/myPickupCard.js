...
function MyPickupCard({ setOpen, setPostId }) {
  const {mypickupdata,setMypickUpdata } = useUserDonationData();

  const [pageNumber, setPageNumber] = useState(0);
  const limit = 6;
  const baseURL = process.env.REACT_APP_API_URL;
  const { fetchPostData } = useFetchPostByUserId();
  const containerRef = useRef(null);
  const [isVisible, setIsVisible] = useState(false);

  const onhandleOpenClosePickUpdetailCard = (boolenValue, postid) => {
    setOpen(boolenValue);
    setPostId(postid);
  };
  ...
  useEffect(() => {
    fetchPostData(
      `${baseURL}/postDonationById/${limit}/${pageNumber}`,
      setIsVisible,setMypickUpdata
    );
  }, [pageNumber]);
  ...
  return (
    <CardGrid>
      {/* {donationData.length} */}
      {mypickupdata?.map((food, id) => (
        <FoodCardDetail
          key={id}
          food={food}
          onhandleOpenClosePickUpdetailCard={onhandleOpenClosePickUpdetailCard}
        />
      ))}
...
const FoodCardDetail = ({
  food,
  onhandleOpenClosePickUpdetailCard,
}) => {
  return (
    <CardContainer>
      ...
      <ActionBar>
        <EditButton
          onClick={() => {
            onhandleOpenClosePickUpdetailCard(true, food?.post_id);
          }}
        >
          More
        </EditButton>
        <DeleteButton>🗑️ {food?.status}</DeleteButton>
      </ActionBar>
    </CardContainer>
  );
};

export default MyPickupCard;
